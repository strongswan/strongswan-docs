...
when prompted for the PIN. With *openssl* we can verify the contents of the generated
certificate request:
----
...
Root CA* which was generated in 2016 with the xref:pki/pki.adoc[`pki`] command
...
----
The {TOOLMAN}/tpm2_nvdefine.1.md[`tpm2_nvdefine`] command allocates a memory location
with a size of 1001 bytes that can be accessed via the handle `*0x01800004*` which
is also called the NV index
----
...
----
The RSA AK private key is attached to the `*charon-systemd*` daemon via the TPM
2.0 TSS interface
----
...
----
The ECC AK private key is attached to the `*charon-systemd*` daemon via the TPM
2.0 TSS interface
----
Jan 04 15:18:38 edu charon-systemd[648407]: TPM 2.0 via TSS2 v2 available
...
----
The ECC AK certificate is loaded by the `*charon-systemd*` daemon via the TPM 2.0
TSS interface
----
Jan 04 15:18:38 edu charon-systemd[648407]: TPM 2.0 via TSS2 v2 available
Jan 04 15:18:38 edu charon-systemd[648407]: loaded certificate from TPM NV index 0x01800004
----
The connection definition is received by the `*charon-systemd*` daemon triggered
by the xref:swanctl/swanctl.adoc[`swanctl --load-conns`] command via the VICI interface
----
