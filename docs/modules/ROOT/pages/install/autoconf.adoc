...
Some directories can be configure through xref:#--with-options[--with options].
...
|--enable-svc                                                |5.2.0
|enable xref:daemons/charonSvc.adoc[charon Windows service]

|--enable-systemd                                            |5.2.1
|enable `systemd` specific IKE daemon
 xref:daemons/charonSystemd.adoc[charon-systemd]
...
|--with-group=GROUP
|xref:features/reducedPrivileges.adoc[change group] of `charon` daemon to `GROUP`
 after startup. [`root`]
...
|--with-user=USER
|xref:features/reducedPrivileges.adoc[change user] of `charon` daemon to USER
 after startup. [`root`]
|===
