...
the package version information stored in the  strongTNC  SQLite database. The
`*dpkg --compare-versions*` command is used to determine which package versions
are affected by a given security update.
