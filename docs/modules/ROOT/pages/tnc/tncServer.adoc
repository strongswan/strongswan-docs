...
defined in the `*TCG*` namespace ({IFMSEG}[TCG TNC IF-M Segmentation]) which
proposes to split up huge PA-TNC (TCG TNC IF-M) messages into
`*TCG/Segmentation Attributes*` with a maxium size of `*32698*` bytes each
----
