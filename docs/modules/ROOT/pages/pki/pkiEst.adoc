...
And overview on the configuration of the
xref:plugins/eap-tls.adoc#_tls_stack_options[TLS stack options] is given
xref:plugins/eap-tls.adoc#_tls_stack_options[here]. It is especially recommended
to set `*version_max = 1.3*` in order to profit from the latest TLS 1.3 version.
