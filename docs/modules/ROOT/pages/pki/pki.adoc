...
commands:
    --gen     (-g)  generate a new private key
...
With version:6.0.5[], the short option for `*--verify*` was changed to `*-V*`
to avoid a conflict with the short option for `*--debug*` (`*-v*`), which can be
used globally since then.
...
Each command has additional options. Pass `*--help*` to a command to get
