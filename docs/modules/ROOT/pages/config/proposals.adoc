...
|*x* |Custom crypto plugin[s]         2+|see `*Plugins*` columns below

|*b* |Botan crypto library             |`*botan*`   | strongSwan 5.7.0

|*w* |wolfSSL crypto library           |`*wolfssl*` | strongSwan 5.8.0

|*o* |OpenSSL or AWS-LC crypto library |`*openssl*` |
...
|12 |x b w o     |m     |sha2, hmac

|*sha384* or *sha2_384*
|SHA2_384_192 HMAC (192 bit)
|13 |x b w o     |m     |sha2, hmac

|*sha512* or *sha2_512*
|SHA2_512_256 HMAC (256 bit)
|14 |x b w o     |m     |sha2, hmac
...
It's also possible to use the hash implementations provided by plugins
...
It's also possible to use the hash implementations provided by plugins together
...
For example, `*x25519-ke1_mlkem768*` adds ML-KEM-768 as additional key
...
technically fine to negotiate e.g. `*x25519-ke7_mlkem768*`).
====
...
|*RSA PKCS#1 v1.5*
|MD5
|1.2.840.113549.1.1.4|*s* x w o   |gmp, md5

|*RSA PKCS#1 v1.5*
|SHA-1
|1.2.840.113549.1.1.5|*s* x b w o |gmp, sha1

|*RSA PKCS#1 v1.5*
|SHA-2 224
|1.2.840.113549.1.1.14|x b w o    |gmp, sha2

|*RSA PKCS#1 v1.5*
|SHA-2 256
|1.2.840.113549.1.1.11|x b w o    |gmp, sha2

|*RSA PKCS#1 v1.5*
|SHA-2 384
|1.2.840.113549.1.1.12|x b w o   |gmp, sha2

|*RSA PKCS#1 v1.5*
|SHA-2 512
|1.2.840.113549.1.1.13|x b w o   |gmp, sha2
