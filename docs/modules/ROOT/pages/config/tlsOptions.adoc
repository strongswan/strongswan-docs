...
xref:plugins/eap-tls.adoc[`*eap-tls*`], `*eap-ttls*`, and `*eap-peap*` plugins,
as well as by the
...
`*Curve448*` are also supported. `CAMELLIA` encryption requires the
`*openssl*` plugin. `NULL` encryption is automatically disabled if
