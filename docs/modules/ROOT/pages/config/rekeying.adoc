= Renewal of IKE and IPsec Security Associations
...
With `IKEv1` it is different as each Quick Mode exchange uses the complete
proposals, so already the first `IPsec SA` will use `PFS` according to the
configuration.
...
configuration issued as described above will be apparent right from the start,
