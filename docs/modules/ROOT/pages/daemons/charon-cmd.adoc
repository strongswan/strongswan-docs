...
|`*--agent*[=<socket>]`
|Use SSH agent for authentication. If socket is not specified, it is read from
...
|`*--childless[=force]*`
|Use childless IKE SA initiation with IKEv2 if supported by the responder,
...
|`*ikev2-psk*`
|IKEv2 with pre-shared key (PSK) authentication
|===
