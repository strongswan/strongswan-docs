...
|Comma-separated list of local traffic selectors to include in CHILD_SA. Each
 ...
 although none of the kernel backends currently supports them except the
 `*kernel-netlink*` plugin  that is able to handle port ranges if they are defined
 by a bit mask (similar to a IP subnet mask) rather than an arbitrary range, see
 the xref:config/quickstart.adoc#_for_specific_port_ranges[following example] for
 details.
 +
