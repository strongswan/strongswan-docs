...
The file uses a xref:config/strongswanConf.adoc#_syntax[`*strongswan.conf*`]-style
...
|<conn>.version            |`2`
|IKE major version to use for connection. `*1*` uses the deprecated IKEv1
 protocol aka ISAKMP, `*2*` uses IKEv2 (the default). A connection using `*0*`
 accepts both IKEv1 and IKEv2 as a responder and initiates the connection
 actively with IKEv2
...
|Since version 5.8.0. Use childless IKE_SA initiation ({RFC6023}[RFC 6023]) for
 ...
 values are `*allow*` (the default), `*prefer*` (since version 5.9.10),
 `*force*` and `*never*`.  If set to `*allow*`, responders will accept childless
 IKE_SAs (as indicated via notify in the IKE_SA_INIT response) while initiators
 continue to create regular IKE_SAs with the first CHILD_SA created during
 IKE_AUTH, unless the IKE_SA is initiated explicitly without any children (which
 will fail if the responder does not support or has disabled this extension).
 ...
 to `*force*`, only childless initiation is accepted in either role. Finally,
 setting the option to `*never*`, disables support for childless IKE_SAs as
 responder
...
|Since version 5.9.14.	Send OCSP status requests in certificate request payloads
 ...
 based authentication. With the default of `*reply*` the daemon sends OCSP status
 responses in certificate payloads if an OCSP status request has been received in
 a certificate request, `*never*` disables sending of OCSP status requests and
 responses altogether, `*request*` causes OCSP status requests in certificate
 request payloads to be sent whenever certificate-based authentication is used,
 `*both*` combines `*reply*` and `*request*`
...
|IKE identity to expect for authentication round. Refer to the `*id*` keyword
 ...
 certificate, either as the subject DN or as a subjectAltName.
 +
 It's possible to use wildcards to match remote identities (e.g. `*{STAR}@strongswan.org*`,
 ...
 +
 Since version 6.0.2, extended POSIX regular expressions are also supported for
 remote identity matching. They must start with an explicit type prefix,
 followed by a caret character (`*&Hat;*`) , and end with a dollar sign (`*$*`)
 to indicate an anchored pattern. Make sure to escape backslash characters when
 configuring identities in double quotes. Supported types are `*rfc822*`,
 `*email*`, `*userfqdn*`, `*fqdn*`, `*dns*`, and `*asn1dn*`. While regular
 expressions are always matched against the string representation of other
 identities, the type must match as well. The matching is performed case
 insensitive. Examples: `*email:&Hat;(moon\|sun)@strongswan\.org$*`,
 `*fqdn:&Hat;vpn[0-9]+\.strongswan\.org$*`, `*"asn1dn:&Hat;.{STAR}CN=.+\\.strongswan\\.org$"*`

|eap_id                    |`id`
|Use EAP-Identity method to request an identity from the client to match against
 and use during EAP authentication. There is currently no "best" match, configs
 are matched in the order they are loaded.
 Wildcards and regular expressions are supported, refer to the `*id*` setting
 for details.
 +
 In versions prior to 6.0.2, there was no matching and an EAP identity was only
 requested if `*%any*` was configured
...
|ESP proposals to offer for the CHILD_SA. A proposal is a set of
 ...
 +
 If a key exchange method is negotiated, CHILD_SA/Quick Mode rekeying and initial
 negotiation use a separate key exchange using the negotiated method. However, for
 IKEv2, the keys of the CHILD_SA created implicitly with the IKE_SA will always be
 derived from the IKE_SA's key material. So any key exchange method specified
 ...
 to fail. If one or more key exchange methods are configured in a proposal, the
 key exchange can be made optional by also adding `*none*`.
 +
 With peers that support multiple IKEv2 key exchanges (RFC 9370), up to seven
 additional key exchanges may be negotiated. They can be configured by prefixing
 the algorithm keyword with `*keX_*` (where `*X*` is a number between 1 and 7).
 Additional key exchanges can be made optional by adding `*keX_none*` to a
 proposal.
 +
 ...
 ESP, the `*default*` set of algorithms for ESP is included. [`default`]
...
|Comma-separated list of local traffic selectors to include in CHILD_SA. Each
 ...
 {RFC4301}[RFC 4301] OPAQUE selectors. Port ranges may be specified as well,
 although only the xref:plugins/kernel-libipsec.adoc[`*kernel-libipsec*`] backend
 directly supports them. The `*kernel-netlink*` plugin is able to handle port
 ranges if they can be mapped to a bit mask (similar to an IP subnet mask) rather
 than an arbitrary range, see the xref:config/quickstart.adoc#_for_specific_port_ranges[following example]
 for details.
 +
 If the protocol is `*icmp*` or `*ipv6-icmp*`, the port is interpreted
 as ICMP message type if it is less than 256 or as type and code if it is greater
 or equal to 256, with the type in the most significant 8 bits and the code in
...
|Comma separated list of remote selectors to include in CHILD_SA. See
 xref:#_connections_conn_children_child_local_ts[`*local_ts*`]
...
|IPsec Mode to establish CHILD_SA with. `*tunnel*` negotiates the CHILD_SA in
 ...
 `*iptfs*` is xref:features/iptfs.adoc[IP-TFS tunnel mode with aggregation and fragmentation]
 (since 6.0.2).
 `*beet*` is the Bound End to End Tunnel mixture mode working with fixed inner
 addresses without the need to include them in each packet. The `*transport*`,
 `*iptfs*`, and `*beet*` modes are subject to mode negotiation. `*tunnel*` mode
 is negotiated if the preferred mode is not available. `*pass*` and `*drop*` are
 used to install shunt policies which explicitly bypass the defined traffic from
 IPsec processing or drop it, respectively. [`tunnel`]
...
|Since version 5.8.0. XFRM interface ID (32-bit unsigned integer) set on inbound
 ...
 with an interface with the same ID. The special value `*%unique*` sets a unique
 interface ID on each CHILD_SA instance, beyond that the value `*%unique-dir*`
 assigns a different unique interface ID for each CHILD_SA direction (in/out)
...
|<child>.per_cpu_sas    |no
|Since version 6.0.2. Enable xref:features/per-cpu-sas.adoc[per-CPU CHILD_SAs].
 Requires `*trap*` in `*start_action*`. The value `*encap*` enables a special
 type of UDP encapsulation (requires enabling `*encap*`` for the connection if
 there is no NAT), where a random source port is used for each outbound per-CPU
 SA (the destination port for all of them remains 4500). This allows using the
 port for RSS if the SPI can't be used. Note that this type of behavior is not
 standardized and not negotiated. So regardless of whether the option is enabled,
 inbound per-CPU SAs with UDP-encapsulation always have the source port set to
 0 as the peer's random port is unknown if it has this option enabled.

|<child>.hw_offload       |`no`
|Enable hardware offload for this CHILD_SA, if supported by the IPsec implementation.
 The values `*crypto*` or `*packet*` (since 5.9.10) enforce crypto or full packet
 offloading and the installation will fail if the selected mode is not supported
 by either kernel or device. On Linux, `*packet*` also offloads policies,
 including trap policies. The value `*auto*` enables full packet or crypto
...
|<child>.icmp             |`no`
|Since version 6.0.5. Whether to forward certain ICMP error messages even if
 their source IP doesn't match the negotiated IPsec policies. ICMP error
 messages, such as Destination Unreachable, Time Exceeded or Fragmentation
 Needed, may be generated by a host whose IP address isn't included in the
 negotiated traffic selectors and therefore doesn't match the IPsec policies.
 If this option is enabled and the kernel supports it (Linux 6.9+), such packets
 may still be forwarded. As ICMP errors contain parts of the IP packet that
 triggered them, the kernel will base its decision on a reverse policy lookup
 using that IP header
...
|Action to perform after loading the configuration. The default of `*none*` loads
 ...
 connection actively. Since version 5.9.6 these two modes can be combined with
 `*trap\|start*`, to immediately initiate a connection for which trap policies
 have been installed.
...
secrets {
    ntlm-carol {
        id = carol
        secret = 0xF7A28E343924D9A06C8183971A7C8373
    }
    ntlm-dave {
        id = dave@strongswan.org
        secret = 0x6E9A3A46012EF27D9DB6F2FF682B1B80
    }
}
----
====
...
----
secrets {
    # multiple identities can be assigned, as long as one of them matches the
    # local or remote identity the secret may be used
    ike-sun-moon {
        id-sun = sun.strongswan.org
        id-moon = moon.strongswan.org
        secret = 0sv+NkxY9LLZvwj4qCC2o/gGrWDF2d21jL
    }
    # this will only be used if sun.strongswan.org is not one of the two
    # identities, otherwise, the PSK above is used
    ike-moon {
        id = moon.strongswan.org
        secret = "This shoud be a strong password"
    }
    ike-carol {
        id = carol@strongswan.org
...
.Example private key passphrase
[%collapsible]
====
[source]
----
secrets {
    # only necessary if the key is encrypted and the passphrase is not passed interactively
    private-carol {
        file = carolKey.pem
...
.Example RSA private key passphrase
[%collapsible]
====
[source]
----
secrets {
    ...
    rsa-dave {
        file = daveKey.pem
        secret = "MeFnDN7VUbj+qU/bkgRIFvbCketIk2wrrs5Ii8297N2v"
    }
}
----
====
...
.Example ECDSA private key passphrase
[%collapsible]
====
[source]
----
...
.Example PKCS#8 private key passphrase
[%collapsible]
====
[source]
----
...
.Example PKCS#12 container passphrase
[%collapsible]
====
[source]
----
secrets {
    # only necessary if passphrase is not passed interactively
    pkcs12-moon {
        file = moonCert.p12
...
----
secrets {
    # PINs may be stored (e.g. for a gateway to access it without interaction)
    token-gw {
        handle = 45
        pin = "12345678"
    }
    # on a client you might want to load the key but enter the pin interactively
    token1 {
        slot = 2
        handle = 45
    }
    # TPM keys may also be loaded
    token_ak_rsa {
        handle = 0x81010003
    }
}
...
|Comma-separated list of additional attributes of type `*<attr>*`. The attribute
 ...
 `*p_cscf*`, `*split_include*` and `*split_exclude*` to define addresses or CIDR
 subnets for the corresponding attribute types. Alternatively, `*<attr>*` can be
 a numerical identifier, for which string attribute values are accepted as well
|===
