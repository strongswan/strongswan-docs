...
certificate and private key. For user certificates, if an alias is configured
instead of distributing a PKCS#12 file, the MDM has to grant permission to the
app to access the certificate/key with that alias. If neither alias nor user
certificate is distributed with the managed profile, the user may select a
...
* Update OpenSSL to support the post-quantum key exchange method ML-KEM. This
  currently requires explicit configuration in proposals (e.g. for IKE something
  like `aes256-sha256-curve25519-ke1_mlkem768`)
* Fix a vulnerability in the eap-mschapv2 plugin
* Fix the filename when exporting the log

=== 2.6.0 (2025-10-01)

* Allow preselecting a user certificate via alias in managed profiles. This
  requires that the MDM grants permission for the alias to the app. If not
  configured, users may select a local certificate manually
* Allow selecting a user certificate for managed profiles that don't install
  their own certificate
* Fix reading split-tunneling settings in managed profiles (`split_tunnelling`
  instead of `split_tunneling` was incorrectly used in the description file)
