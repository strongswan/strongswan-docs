...
We store the captured `*wireshark*` trace under the file name `*moon.pcapng*`.
...
image::wiresharkESP.png[Encrypted wireshark trace, link=self]
...
image::wiresharkIKE.png[Encrypted wireshark trace, link=self]
