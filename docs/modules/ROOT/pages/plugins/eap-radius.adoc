...
image:a-m-c-w-d.png[topology,252,129,link={EX1},title=rw-eap-framed-ip-radius example]
strongSwan {EX1}[example] using the {RFC2865}#section-5.8[`*Framed-IP-Address*`] attribute

image:a-m-c-w-d.png[topology,252,129,link={EX2},title=rw-eap-md5-class-radius example]
...
image:a-m-c.png[topology,187,129,link={EX3},title=rw-radius-accounting example]
...
image:a-m-c.png[topology,187,129,link={EX4},title=xauth-rsa-radius example]
strongSwan {EX4}[example] showing the use of `*XAuth*` with the `*eap-radius*`
plugin.
...
----
The native iOS and OS X clients are known to work fine with multiple
