...
configuration in a relational database. Further, the `*charon*` daemon reads
credentials such as certificates, private keys or passwords from the database
to do all kinds of authentication. Logging to the database is also possible.
...
can be found xref:./sqlite.adoc[here]).
